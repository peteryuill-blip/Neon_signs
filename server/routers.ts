...
export const appRouter = router({
  ...
  export: router({
    ...
    unifiedCsv: protectedProcedure.query(async ({ ctx }) => {
      ...
      // --- Helper: detect deferred/placeholder text ---
      ...
      // --- Helper: parse Works Made into Started / Finished / Notes ---
      ...
      // --- Helper: split Technical Intent into Observation vs Self-Directive ---
      function splitTechnicalIntent(text: string | null | undefined): { observation: string; directive: string } {
        ...
        // If it contains directive language, try to split on comma/period
        if (hasDirective) {
          ...
          // Can't cleanly split — default to observation
          return { observation: cleaned, directive: '' };
        }

        // Default: everything is observation
        return { observation: cleaned, directive: '' };
      }

      // =============================================
      // FILE 1: WEEKLY ROUNDUP section
      // =============================================
      ...
      // =============================================
      // FILE 2: CRUCIBLE TRIALS section
      // =============================================
      const trialHeaders = [
